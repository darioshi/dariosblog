			...
			<div class="list-group">
				...
				<c:forEach var="map" items="${sort_article_map}">
                    <div>
                        <div class="sort_name">
                            <!-- 分类名字 -->
                            <span class="glyphicon glyphicon-triangle-bottom"></span>&nbsp;&nbsp;<input type="text"  id="sort_${map.key.id}" value="${map.key.name}"   disabled="disabled" style="border:0px; height: 30px; font-size: 20px;">
                            <c:if test="${sessionScope.user!=null}">
                            <button class="btn btn-default"><span class="glyphicon glyphicon-pencil" style="color:#5bc0de" onclick="edit_sort(this,'${map.key.id}')">编辑</span></button>
                            <button class="btn btn-default"><span class="glyphicon glyphicon-trash" style="color:#d9534f" onclick="delet_sort(this,'${map.key.id}')">删除</span></button>
                            </c:if>
                        </div>
        ...
        if (confirm("确定删除文章吗?")) {
            ...
            xmlhttp.onreadystatechange = function () {
                if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                    ...
                    if(obj.code == 1) {
                        //remove 视图
                        var recorder = ele.parentNode.parentNode.parentNode.parentNode;
